...
namespace GtkSharp.Generation {
	...
	public class Method : MethodBase  {
		...
		public override bool Validate ()
		{
			...
			IsGetter = ((((parms.IsAccessor && retval.IsVoid) || (parms.Count == 0 && !retval.IsVoid)) || (parms.Count == 0 && !retval.IsVoid)) && HasGetterName);
		...
		void GenerateDeclCommon (StreamWriter sw, ClassBase implementor)
		{
			if (IsStatic)
				sw.Write("static ");
			sw.Write (Safety);
		...
		public void GenerateBody (GenerationInfo gen_info, ClassBase implementor, string indent)
		{
			...
			Body.Initialize(gen_info, IsGetter, is_set, indent);
