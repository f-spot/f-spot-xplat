	...
	public abstract class ClassBase : GenBase {
		...
		public override bool Validate ()
		{
			...
			foreach (string iface in interfaces) {
				var igen = SymbolTable.Table[iface] as InterfaceGen;
				if (igen == null) {
		...
		void ParseImplements (XmlElement member)
		{
		...
		public virtual Method GetMethodRecursively (string name, bool check_self)
		{
			...
			if (check_self && p == null) {
				foreach (string iface in interfaces) {
					var igen = SymbolTable.Table.GetClassGen (iface);
					if (igen == null)
						continue;
		...
		void InitializeCtors ()
		{
			...
			var valid_ctors = new ArrayList();
			clash_map = new Hashtable();

			foreach (Ctor ctor in ctors) {
				if (clash_map.Contains (ctor.Signature.Types)) {
					var clash = clash_map [ctor.Signature.Types] as Ctor;
