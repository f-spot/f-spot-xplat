...
namespace GtkSharp.Generation {
	...
	public class ArrayParameter : Parameter {
		...
		{
			NullTerminated = elem.HasAttribute ("null_term_array");
		}
        ...
        public override string[] Prepare {
			get {
				...
				var result = new ArrayList {
					string.Format ("int cnt_{0} = {0} == null ? 0 : {0}.Length;", CallName),
					string.Format ("{0}[] native_{1} = new {0} [cnt_{1}" + (NullTerminated ? " + 1" : "") + "];", MarshalType.TrimEnd ('[', ']'), CallName),
					string.Format ("for (int i = 0; i < cnt_{0}; i++)", CallName)
				};
				var gen = Generatable;
				if (gen is IManualMarshaler)
	...
	public class ArrayCountPair : ArrayParameter {
		readonly XmlElement count_elem;
		...
		string CallCount (string name)
		{
			...
			var gen = SymbolTable.Table[count_elem.GetAttribute("type")];
