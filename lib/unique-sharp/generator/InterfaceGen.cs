...
namespace GtkSharp.Generation {
	...
	public class InterfaceGen : ObjectBase {
		readonly ArrayList vms = new ArrayList ();
		readonly ArrayList members = new ArrayList ();

		public InterfaceGen (XmlElement ns, XmlElement elem) : base (ns, elem) 
		{
			IsConsumeOnly = elem.HasAttribute ("consume_only");
			foreach (XmlNode node in elem.ChildNodes) {
				switch (node.Name) {
				case "virtual_method":
					var vm = new VirtualMethod (node as XmlElement, this);
					vms.Add (vm);
					members.Add (vm);
					break;
				case "signal":
		...
		{
			var sw = gen_info.Writer = gen_info.OpenStream (Name + "Adapter");
			...
			var temp = methods ["GetType"] as Method;
			if (temp != null)
			...
			if (File.Exists (custom)) {
				...
				using (var sr = new StreamReader(new FileStream (custom, FileMode.Open, FileAccess.Read)))
		...
		void GenerateImplementorIface (StreamWriter sw)
		{
			...
			foreach (VirtualMethod vm in vms) {
				if (vm_table [vm.Name] == null)
					...
					var cmp = vm_table [cmp_name] as VirtualMethod;
		...
		public override void Generate (GenerationInfo gen_info)
		{
			...
			var sw = gen_info.Writer = gen_info.OpenStream (Name);
