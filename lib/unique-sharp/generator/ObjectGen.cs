...
namespace GtkSharp.Generation {
	...
	public class ObjectGen : ObjectBase  {
		readonly ArrayList custom_attrs = new ArrayList();
		readonly ArrayList strings = new ArrayList();
		readonly ArrayList vm_nodes = new ArrayList();
		readonly Hashtable childprops = new Hashtable();
		static readonly Hashtable dirs = new Hashtable ();
		...
		static DirectoryInfo GetDirectoryInfo (string dir, string assembly_name)
		{
		...
		public override void Generate (GenerationInfo gen_info)
		{
			...
			var di = GetDirectoryInfo (gen_info.Dir, asm_name);
			...
			if (!has_sigs) {
				foreach (string iface in interfaces) {
					var igen = table.GetClassGen (iface);
			...
			if (interfaces.Count != 0) {
				var all_methods = new Hashtable ();
				foreach (Method m in Methods.Values)
					all_methods[m.Name] = m;
				var collisions = new Hashtable ();
				foreach (string iface in interfaces) {
					var igen = table.GetClassGen (iface);
					foreach (Method m in igen.Methods.Values) {
						var collision = all_methods[m.Name] as Method;
		...
		protected void GenChildProperties (GenerationInfo gen_info)
		{
			...
			var child_ancestor = Parent as ObjectGen;
		...
		void GenVMGlue (GenerationInfo gen_info, XmlElement elem)
		{
		...
		void GenVirtualMethods (GenerationInfo gen_info)
		{
			if (vmhdrs_needed) {
		...
		static bool NeedsMap (Hashtable objs, string assembly_name)
		{
		...
		public static void GenerateMappers ()
		{
			foreach (string dir in dirs.Keys) {
				...
				var di = dirs[dir] as DirectoryInfo;

				if (!NeedsMap (di.objects, di.assembly_name))
					continue;
	
				var gen_info = new GenerationInfo (dir, di.assembly_name);

				GenerateMapper (di, gen_info);
			}
		}

		static void GenerateMapper (DirectoryInfo dir_info, GenerationInfo gen_info)
		{
			var sw = gen_info.OpenStream ("ObjectManager");
