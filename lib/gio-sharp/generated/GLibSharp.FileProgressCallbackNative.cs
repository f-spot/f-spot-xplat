...
delegate void FileProgressCallbackNative(long current_num_bytes, long total_num_bytes, IntPtr user_data);

	class FileProgressCallbackInvoker {
		readonly FileProgressCallbackNative native_cb;
	...
	class FileProgressCallbackWrapper {
		...
		readonly GLib.FileProgressCallback managed;
		...
		public static GLib.FileProgressCallback GetManagedDelegate (FileProgressCallbackNative native)
		{
			if (native == null)
				return null;
			var wrapper = (FileProgressCallbackWrapper) native.Target;
			if (wrapper == null)
				return null;
