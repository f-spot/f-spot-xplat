...
delegate IntPtr ReallocFuncNative(IntPtr data, UIntPtr size);

	class ReallocFuncInvoker {
		readonly ReallocFuncNative native_cb;
		...
		{
			var result = native_cb (__data, new UIntPtr (size));
			return result;
		}
	}

	class ReallocFuncWrapper {

		public IntPtr NativeCallback (IntPtr data, UIntPtr size)
		{
			try {
				var __ret = managed (data, (ulong) size);
				if (release_on_call)
					gch.Free ();
				return __ret;
			} catch (Exception e) {
		...
		readonly GLib.ReallocFunc managed;
		...
		public static GLib.ReallocFunc GetManagedDelegate (ReallocFuncNative native)
		{
			if (native == null)
				return null;
			var wrapper = (ReallocFuncWrapper) native.Target;
			if (wrapper == null)
				return null;
