...
delegate bool FileReadMoreCallbackNative(IntPtr file_contents, long file_size, IntPtr callback_data);

	class FileReadMoreCallbackInvoker {
		readonly FileReadMoreCallbackNative native_cb;
	...
	class FileReadMoreCallbackWrapper {
		...
		readonly GLib.FileReadMoreCallback managed;
		...
		public static GLib.FileReadMoreCallback GetManagedDelegate (FileReadMoreCallbackNative native)
		{
			if (native == null)
				return null;
			var wrapper = (FileReadMoreCallbackWrapper) native.Target;
			if (wrapper == null)
				return null;
